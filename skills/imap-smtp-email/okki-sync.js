...
const CONFIG = {
  ...
  TRAIL_TYPE: {
    QUOTATION: 101,      // 快速记录
    EMAIL: 102,          // 邮件
    ORDER_CONTRACT: 103, // 订单/合同生成（复用电话类型）
    MEETING: 104,        // 会面
    SOCIAL: 105          // 社交平台
  }
};
 ...
 * 创建订单/合同跟进记录
 * @param {string} companyId - OKKI 客户 ID
 * @param {object} orderContractData - 订单/合同数据
 * @param {string} orderContractData.uid - 订单/合同 UID（用于去重）
 * @param {string} orderContractData.orderContractNo - 订单/合同编号
 * @param {string} orderContractData.date - 日期
 * @param {string} orderContractData.type - 类型 ('PI' 或 'Contract')
 * @param {Array} orderContractData.products - 产品列表
 * @param {string} orderContractData.totalAmount - 总金额
 * @param {string} orderContractData.validUntil - 有效期
 * @param {Array} orderContractData.filePaths - 生成的文件路径
 * @returns {Promise<object>}
 */
async function createOrderContractTrail(companyId, orderContractData) {
  // 去重检查
  if (orderContractData.uid && isProcessed(orderContractData.uid)) {
    return {
      success: false,
      reason: 'duplicate',
      message: '订单/合同已处理',
      uid: orderContractData.uid
    };
  }
  
  // 构建产品列表字符串
  const productList = orderContractData.products && orderContractData.products.length > 0
    ? orderContractData.products.map(p => `  - ${p.name || p.description} x ${p.quantity || 1}`).join('\n')
    : '(未列出产品)';
  
  const fileList = orderContractData.filePaths && orderContractData.filePaths.length > 0
    ? `\n文件：${orderContractData.filePaths.join(', ')}`
    : '';
  
  const content = `${orderContractData.type} 生成\n` +
    `编号：${orderContractData.orderContractNo}\n` +
    `日期：${orderContractData.date}\n` +
    `总金额：${orderContractData.totalAmount || '(未指定)'}\n` +
    `有效期：${orderContractData.validUntil || '(未指定)'}\n` +
  ...
  try {
    const result = await execOkkiCli([
      ...
      '--company', companyId,
      '--content', content,
      '--type', CONFIG.TRAIL_TYPE.ORDER_CONTRACT.toString()
    ]);
    ...
    // 记录已处理
    if (orderContractData.uid) {
      saveProcessedRecord(orderContractData.uid, {
        type: 'order_contract',
        company_id: companyId,
        trail_id: result.trail_id || result.data?.trail_id,
        order_contract_no: orderContractData.orderContractNo,
        doc_type: orderContractData.type
      });
    }
...
if (require.main === module) {
  ...
  if (command === 'test') {
  ...
  } else if (command === 'order-contract') {
    // 用法：node okki-sync.js order-contract <json_string>
    // json_string 包含：{ "dataFile": "path/to/data.json", "orderContractNo": "ORD-xxx", "type": "PI" }
    if (!args[1]) {
      console.log('用法：node okki-sync.js order-contract <json_string>');
      console.log('json_string 示例：{"dataFile":"/path/to/data.json","orderContractNo":"ORD-20260326-001","type":"PI"}');
      process.exit(1);
    }
    ...
    (async () => {
      try {
        const params = JSON.parse(args[1]);
        const dataFile = params.dataFile;
        const orderContractNo = params.orderContractNo;
        const docType = params.type; // PI or Contract
        ...
        if (!docType) {
          console.error('❌ 缺少 type 参数 (PI 或 Contract)');
          process.exit(1);
        }
        ...
        if (!customerEmail && !customerName) {
          console.warn('⚠️  数据文件中缺少 customer.email 和 customer.name，无法匹配客户');
          // 记录未匹配
          fs.appendFileSync(CONFIG.unmatchedLog, `[${new Date().toISOString()}] order-contract ${orderContractNo || 'unknown'}: no customer email/name\n`);
          process.exit(0);
        }
        ...
        if (!customer) {
          console.warn(`⚠️  未找到匹配客户：${customerEmail || customerName}`);
          fs.appendFileSync(CONFIG.unmatchedLog, `[${new Date().toISOString()}] order-contract ${orderContractNo}: ${customerEmail || customerName} - no match\n`);
          process.exit(0);
        }
        ...
        const orderContractData = {
          uid: `${docType.toLowerCase()}-${Date.now()}-${orderContractNo || 'unknown'}`,
          orderContractNo: orderContractNo || data.orderContractNo || data.order?.order_no || 'unknown',
          date: data.date || data.order?.date || new Date().toISOString().split('T')[0],
          type: docType,
          products: products.map(p => ({
            name: p.description || p.name || p.product_name,
            quantity: p.quantity,
            unit_price: p.unit_price || p.unitPrice
          })),
          totalAmount: `${data.currency || 'USD'} ${totalAmount.toFixed(2)}`,
          validUntil: data.validUntil || data.order?.valid_until || data.order?.validUntil || '(未指定)',
          filePaths: params.filePaths || []
        };
        
        // 创建跟进记录
        console.log(`📝 创建 ${docType} 跟进记录：${orderContractData.orderContractNo}...`);
        const result = await createOrderContractTrail(customer.company_id, orderContractData);
    ...
    console.log('用法：node okki-sync.js test | quotation <json_string> | order-contract <json_string>');
    console.log('模块导出：matchCustomer, createEmailTrail, createQuotationTrail, createOrderContractTrail, syncEmailToOkki');
  }
}
