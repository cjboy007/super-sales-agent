...
Intelligent email auto-reply pipeline for `sale-9@farreach-electronic.com`. Automatically classifies incoming emails by intent, retrieves relevant knowledge from the Farreach knowledge base, generates personalized reply drafts, and routes them through a Discord-based human review workflow before sending.

**Pipeline:** IMAP fetch → Intent Recognition → KB Retrieval → Reply Generation → Discord Review → SMTP Send

This skill is domain-specific to Farreach Electronic's B2B email workflow. It is NOT a generic email assistant — it understands product lines (HDMI/DP/USB/LAN cables), customer intent categories specific to electronics manufacturing, and integrates with OKKI CRM data.
...
// Draft saved to: /Users/wilson/.openclaw/workspace/skills/imap-smtp-email/drafts/
```
...
### External Services
- **IMAP/SMTP:** `sale-9@farreach-electronic.com` via 网易企业邮 (configured in `imap-smtp-email/.env`)
...
### Local Skills/Tools
- `/Users/wilson/.openclaw/workspace/skills/imap-smtp-email/` — IMAP/SMTP transport layer
- `/Users/wilson/.openclaw/workspace/vector_store/okki_vector_search_v3.py` — LanceDB vector search
- `/Users/wilson/.openclaw/workspace/obsidian-vault/Farreach 知识库/` — Product knowledge base
