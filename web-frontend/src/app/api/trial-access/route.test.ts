...
describe("/api/trial-access", () => {
  it("sends a phone code without leaking it in the response", async () => {
    ...
    const response = await POST(post("http://localhost/api/trial-access/send-code", { phone: "13680342402" }));
  ...
  it("verifies a phone code and sets a trial session cookie for 14 days", async () => {
    ...
    await sendRoute.POST(post("http://localhost/api/trial-access/send-code", { phone: "13680342402" }));
