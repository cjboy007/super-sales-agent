...
describe("/api/worker-supervisor route", () => {
  ...
  it("prepares worker recovery without activation admin auth", async () => {
    const { POST } = await import("./route");
    ...
    expect(fs.existsSync(path.join(tempRoot, "runtime", "supervisors", "ssa-jaden-hero-pumps-1.supervisor.json"))).toBe(true);
  });
