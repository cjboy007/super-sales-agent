...
beforeEach(() => {
  vi.resetModules();
  vi.restoreAllMocks();
  tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "ssa-inbox-route-test-"));
...
describe("/api/inbox route", () => {
  it("serves local inbox memory without auditing an external fetch when the bridge is disabled", async () => {
    ...
    const response = await GET(request("http://localhost/api/inbox?project=farreach"));
  ...
  it("audits and blocks Farreach inbox bridge fetches by default", async () => {
    process.env.SSA_ENABLE_FARREACH_BRIDGE = "true";
    const fetchMock = vi.spyOn(globalThis, "fetch");
    const { GET } = await import("./route");

    const response = await GET(request("http://localhost/api/inbox?project=farreach&limit=5"));
    const json = await response.json();
  ...
  it("calls the Farreach inbox bridge only when IMAP side effects are explicitly enabled", async () => {
    process.env.SSA_ENABLE_FARREACH_BRIDGE = "true";
    ...
    expect(fetchMock).toHaveBeenCalledWith("http://farreach.test/api/v1/inbox?limit=5", {
      next: { revalidate: 30 },
    });
