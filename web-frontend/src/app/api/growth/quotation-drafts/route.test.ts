...
describe("/api/growth/quotation-drafts routes", () => {
  it("enforces workspace-scoped auth on reads and draft creation", async () => {
    const listRoute = await import("./route");
    const draftRoute = await import("./draft/route");

    expect((await listRoute.GET(request("http://localhost/api/growth/quotation-drafts?project=farreach"))).status).toBe(403);
    expect((await draftRoute.POST(request("http://localhost/api/growth/quotation-drafts/draft?project=farreach", {
      method: "POST",
      body: { prospectingPacketId: "blocked" },
    }))).status).toBe(403);

    const allowed = await listRoute.GET(request("http://localhost/api/growth/quotation-drafts?project=hero-pumps"));
