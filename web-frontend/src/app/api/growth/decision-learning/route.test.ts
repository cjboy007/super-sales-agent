...
describe("/api/growth/decision-learning routes", () => {
  it("returns decision-learning state for explicitly selected workspaces without activation auth", async () => {
    const listRoute = await import("./route");

    const farreach = await listRoute.GET(request("http://localhost/api/growth/decision-learning?project=farreach"));
    const farreachJson = await farreach.json();
    const hero = await listRoute.GET(request("http://localhost/api/growth/decision-learning?project=hero-pumps"));
