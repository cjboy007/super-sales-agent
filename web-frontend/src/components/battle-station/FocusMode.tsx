...
export default function FocusMode({
  ...
  return (
    <div className="flex h-screen flex-col overflow-hidden bg-slate-950 text-slate-200">
      <header className="flex h-11 shrink-0 items-center justify-between border-b border-slate-800 bg-slate-900/75 px-4">
        <div className="flex min-w-0 items-center gap-3">
          <button
            type="button"
            onClick={onBack}
            className="shrink-0 text-xs font-semibold text-slate-500 transition hover:text-slate-200"
          >
            {copy.back}
          </button>
          <span className="text-slate-700">/</span>
          <h1 className="truncate text-xs font-semibold text-amber-300">
            {copy.focusPrefix} {focusCase.title}
          </h1>
        ...
        <div className="flex shrink-0 items-center gap-3">
          <span className="font-mono text-[10px] text-slate-500">{approvalState}</span>
