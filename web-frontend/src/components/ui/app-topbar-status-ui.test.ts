...
describe("global task progress status placement", () => {
  it("binds the SSA runtime indicator to the task progress nav item instead of a floating widget", () => {
    expect(topBarSource).toContain("opsStatus");
    expect(topBarSource).toContain("ops-status-dot");
    expect(topBarSource).toContain("item.href === \"/agent-status\"");
    expect(topBarSource).toContain("const label = language === \"zh\" ? item.zhLabel ?? item.label : item.label");
