...
function getHeroIronTasks(): AgentTask[] {
  const sentLogPath = path.join(PROJECT_ROOT, "..", "hero-pumps", "sent-log.json");
...
function getHeroOracleTasks(): AgentTask[] {
  const trendsPath = path.join(PROJECT_ROOT, "data", "intelligence", "trends.json");
  const competitorsPath = path.join(PROJECT_ROOT, "data", "intelligence", "competitors.json");
