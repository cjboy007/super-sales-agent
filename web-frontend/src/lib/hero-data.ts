import { requestSideEffect } from "./runtime/side-effect-gate";

// Fetches real-time data from Mac Mini via SSH reverse tunnel when explicitly enabled.
...
async function fetchJson<T>(endpoint: string): Promise<T | null> {
  const sideEffect = requestSideEffect({
    kind: "data.read",
    workspaceId: "hero-pumps",
    summary: `Read Hero Pumps data API ${endpoint}`,
    payload: {
      source: "hero-data",
      endpoint: DATA_API,
      path: endpoint,
...
export async function isDataApiAvailable(): Promise<boolean> {
  const sideEffect = requestSideEffect({
    kind: "data.read",
    workspaceId: "hero-pumps",
    summary: "Read Hero Pumps data API health",
    payload: {
      source: "hero-data.health",
      endpoint: DATA_API,
      path: "/health",
    },
