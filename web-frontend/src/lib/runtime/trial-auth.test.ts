...
beforeEach(() => {
  ...
  process.env.SSA_TRIAL_EXPIRED_CONTACT_PHONE = "13680342402";
...
describe("trial auth", () => {
  it("sends a mainland China mobile verification code and opens one 14-day trial", async () => {
    ...
    const sent = await requestTrialSmsCode({ phone: "+8613680342402", ip: "203.0.113.10" });
    expect(sent).toMatchObject({ ok: true, phone: "13680342402", expiresInSeconds: 300 });

    const verified = await verifyTrialSmsCode({
      phone: "13680342402",
      code: storedCodeFor("13680342402"),
      ip: "203.0.113.10",
    });

    expect(verified.ok).toBe(true);
    if (verified.ok) {
      expect(verified.session.phone).toBe("13680342402");
      ...
      expect(verified.session.contactPhone).toBe("13680342402");
  ...
  it("does not refresh the two-week trial when the same phone verifies again", async () => {
    ...
    const first = await verifyTrialSmsCode({ phone: "13680342402", code: storedCodeFor("13680342402"), ip: "203.0.113.10" });
    vi.setSystemTime(new Date("2026-06-20T00:00:00.000Z"));
    await requestTrialSmsCode({ phone: "13680342402", ip: "203.0.113.10" });
    const second = await verifyTrialSmsCode({ phone: "13680342402", code: storedCodeFor("13680342402"), ip: "203.0.113.10" });
  ...
  it("uses Aliyun PNVS to verify externally generated SMS codes", async () => {
    ...
    const sent = await requestTrialSmsCode({ phone: "13680342402", ip: "203.0.113.10" });
    const verified = await verifyTrialSmsCode({ phone: "13680342402", code: "654321", ip: "203.0.113.10" });
    const sendBody = String(fetchMock.mock.calls[0][1]?.body);
    const verifyBody = String(fetchMock.mock.calls[1][1]?.body);

    expect(sent).toMatchObject({ ok: true, phone: "13680342402" });
    expect(verified.ok).toBe(true);
  ...
  it("rejects expired trials and returns the local deployment contact phone", async () => {
    ...
    const sendAgain = await requestTrialSmsCode({ phone: "13680342402", ip: "203.0.113.10" });

    expect(session.ok).toBe(false);
    if (!session.ok) {
      expect(session.reason).toBe("trial_expired");
      expect(session.contactPhone).toBe("13680342402");
    }
    expect(sendAgain).toMatchObject({ ok: false, reason: "trial_expired", contactPhone: "13680342402" });
  });

  it("protects a small server with phone cooldown, IP SMS cap, and active trial caps", async () => {
    const { requestTrialSmsCode, verifyTrialSmsCode } = await import("./trial-auth");

    const firstSend = await requestTrialSmsCode({ phone: "13680342402", ip: "203.0.113.10" });
    const cooldown = await requestTrialSmsCode({ phone: "13680342402", ip: "203.0.113.10" });
    ...
    await verifyTrialSmsCode({ phone: "13680342402", code: storedCodeFor("13680342402"), ip: "203.0.113.10" });
  ...
  it("enforces daily SMS caps per phone and per IP", async () => {
    ...
    const phoneLimited = await requestTrialSmsCode({ phone: "13680342402", ip: "203.0.113.10" });
  ...
  it("limits heavy trial operations per phone per day", async () => {
    ...
    await requestTrialSmsCode({ phone: "13680342402", ip: "203.0.113.10" });
    const verified = await verifyTrialSmsCode({ phone: "13680342402", code: storedCodeFor("13680342402"), ip: "203.0.113.10" });
    expect(verified.ok).toBe(true);
