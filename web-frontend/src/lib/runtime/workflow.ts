...
import type { CompanyIntelEnrichment, CompanyIntelLeadInput, CompanyIntelReadModel } from "./company-intel";
...
interface WorkflowRuntimeHost {
  ...
  collectCompanyIntel(input: {
    workspaceId: WorkspaceId;
    lead: CompanyIntelLeadInput;
  }): Promise<CompanyIntelEnrichment>;
...
export class WorkflowEngine {
  ...
  enqueue(workspaceId: WorkspaceId, workflow: RuntimeWorkflowType, input: Record<string, unknown>): RuntimeJob {
    ...
    const steps = workflow === "company_intel.run"
      ? [
        ...
        makeStep("channels", "memory", "Run all available company-intel channels"),
  ...
  async run(jobId: string): Promise<RuntimeJob> {
    ...
    try {
      if (job.workflow === "company_intel.run") {
        return await this.runCompanyIntelJob(job);
      }
  ...
  private async runCompanyIntelJob(job: RuntimeJob): Promise<RuntimeJob> {
    ...
    const enrichment = await this.host.collectCompanyIntel({
      workspaceId: job.workspaceId,
      lead,
    });
    job = this.completeStep(job, "channels", {
      officialWebsite: enrichment.website || null,
      mx: enrichment.mx || null,
    });
