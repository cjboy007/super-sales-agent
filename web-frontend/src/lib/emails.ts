...
import { getDraftRecords } from "./db";

const SENT_LOG = paths.heroSentLog;
const MAIL_ARCHIVE = paths.mailArchive;
const DRAFTS_PATH = paths.heroTemplates;
...
function getTemplateDrafts(): EmailDraft[] {
  try {
...
/**
 * 获取草稿：SSA-owned DB first, file templates as compatibility fallback.
 */
export function getEmailDrafts(): EmailDraft[] {
  const runtimeDrafts = getDraftRecords(undefined, 100).map((draft) => ({
    id: draft.id,
    subject: draft.subject,
    template: draft.template || draft.source,
  }));
  const seen = new Set(runtimeDrafts.map((draft) => draft.id));
  const templateDrafts = getTemplateDrafts().filter((draft) => !seen.has(draft.id));
  return [...runtimeDrafts, ...templateDrafts];
}
