...
export type AgentEventType = "agent-update" | "email-progress" | "new-lead" | "email-sent" | "research-complete";
...
export function publishAndRemember(event: Omit<AgentEvent, "id" | "timestamp">): AgentEvent {
  ...
  try {
    createAuditEvent({
      ...
      actor: "events",
      target: event.type,
      summary: `Event published: ${event.type}`,
      metadata: event.data,
    });
  } catch {
    // Audit logging is best-effort; event publication still succeeds.
  }
  return full;
}
