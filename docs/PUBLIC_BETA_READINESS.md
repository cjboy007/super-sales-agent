...
export SSA_TRIAL_EXPIRED_CONTACT_PHONE=13680342402
...
the app shows `13680342402` on expired, full, or closed trial states.
...
`npm run start:standalone`. The standalone script copies `.next/static` and the
`public/` directory into the standalone bundle before starting so browser assets,
icons, and the web manifest are served correctly.
...
Demo seed defaults to the `demo-exporter` workspace. Do not seed demo data into
real project workspaces such as `farreach` or `hero-pumps`; those are protected
by default. If an operator intentionally needs a temporary drill on a protected
workspace, set `SSA_ALLOW_DEMO_ON_PROTECTED_WORKSPACES=true` only for that
session, then unset it.

After seeding, `/leads?project=demo-exporter` should show demo customers, a
primary contact, an inbound email activity, and PI/order milestones.
`/agent-status` should show which beta readiness items are ready and which still
need setup.
